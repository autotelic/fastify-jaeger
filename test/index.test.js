...
// This test should throw an error -> "this._reporter.setProcess is not a function"
test('Should initialize plugin with user configuration', async ({ teardown, is, has }) => {
  delete require.cache[require.resolve('jaeger-client')]
  delete require.cache[require.resolve(require('path').join(__dirname, '../index'))]
  require('jaeger-client')
  const initTracerSpy = spy(require.cache[require.resolve('jaeger-client')].exports.initTracer)
  ...
  fastify.register(require('../index'), { serviceName: 'test',  reporter: { logSpans: true } })
